...
switch ($env:BROWSER)
{
  chrome
  {
    ...
    $webdriverprocess = Start-Job -Init ([ScriptBlock]::Create("Set-Location '$pwd'")) -ScriptBlock { chromedriver --port=4444 --log-level=INFO --enable-chrome-logs *>&1 >$using:loglocation\webdriver.log }
